 ...
 * 测行高并建立 y 布局。
 * 规则尽量少：
 *  - w/h 必须 > 0（侧栏折叠 / display:none 时拒测）
 *  - offsetHeight 为 0 的行跳过累计失败
 *  - scale 是 transform，不改 offsetHeight，无需清 transform
 */
...
/**
 * 打开歌词面板后的测量入口。
 * 立刻测一次（能测就定位），再在过渡结束后测一次。
 * soft 参数保留兼容，行为相同——不再搞多 timer 叠重试。
 */
