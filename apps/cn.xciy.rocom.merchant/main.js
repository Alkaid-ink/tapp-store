...
async function syncRocom(force) {
  ...
  rocomSyncPromise = (async function () {
    ...
    if (merchantResult.status !== 'fulfilled') {
      ...
      return { state: 'unavailable', products: [], activities: previous && previous.activities || [], announcements: previous && previous.announcements || [], round: getRound(), updatedAt: previous && previous.updatedAt || Date.now(), watchCount: 0 };
    }
    var merchant = normalizeMerchant(unwrap(merchantResult.value), config.watchlist); var snapshot = { state: merchant.products.length ? 'ready' : 'empty', activityName: text(merchant.activity.name || '\u8fdc\u884c\u5546\u4eba'), subtitle: text(merchant.activity.start_date || '\u6bcf\u65e5 08:00 \u81f3 24:00\uff0c\u6bcf 4 \u5c0f\u65f6\u6362\u8d27'), products: merchant.products, activities: results[1].status === 'fulfilled' ? activityItems(unwrap(results[1].value)) : (previous && previous.activities || []), announcements: results[2].status === 'fulfilled' ? announcementItems(unwrap(results[2].value)) : (previous && previous.announcements || []), watchCount: merchant.products.filter(function (item) { return item.watched; }).length, round: getRound(), updatedAt: Date.now() };
...
if (Tapp.pages) {
  ...
  Tapp.lifecycle.onReady(function () { var root = typeof document !== 'undefined' ? document.querySelector('[data-page-root]') : null; if (root) { Tapp.pages['rocom-merchant-dashboard'].render(root); } });
}
